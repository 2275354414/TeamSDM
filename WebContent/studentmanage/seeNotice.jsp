...
<html>
  <head>
	...
	<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/themes/default/easyui.css">
	<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/themes/icon.css">
	<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/demo/demo.css">
	<!-- <link rel="stylesheet" type="text/css" href="js/jquery-easyui-1.3.3/themes/bootstrap/easyui.css"> -->
	<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/jquery-1.8.0.min.js" charset="utf-8"></script>
	<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/jquery.easyui.min.js" charset="utf-8"></script>
	<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/locale/easyui-lang-zh_CN.js" charset="utf-8"></script>
	...
	<script type="text/javascript">
		...
		function toChangeDorm(row){
	    	$.post("${pageContext.request.contextPath}/qryStudentChangeDorm.do?student_userName="+row.send_userName+"&guid="+row.guid,null,function(data){
	    		...
	    		$("#guid").val(resultData[0].guid);
	    	});
	    	
	    	// 查询出全部的building列表
			$.post("${pageContext.request.contextPath}/queryAllBuilding_changeDorm.do?index="+Math.random(),null,function(data){
		...
		function changeBuilding_changeDorm(id){
			...
			if(building_id!=""){
				$.post("${pageContext.request.contextPath}/queryDormByBuilding_changeDorm.do",
		...
		function saveChange(){
			...
			$("#fm_changeDorm").form("submit",{
				url:"${pageContext.request.contextPath}/change.do?student_name="+student_name,
				onSubmit:function(){
		...
		function saveOut(){
			...
			var notice_guid = $("#notice_guid").val();
			$("#fm_outDorm").form("submit",{
				url:"${pageContext.request.contextPath}/outDorm.do?guid="+notice_guid+"&student_id="+student_id+"&out_date="+out_date+"&student_remark="+student_remark,
				onSubmit:function(){
					return $(this).form('validate');
				},
				success:function(result){
  ...
  <table class=easyui-datagrid id="dg" title="消息基本信息"
       toolbar="#toolbar" pagination="true" pageList=[5,10,15] 
    	...
    	url="${pageContext.request.contextPath}/qryNotSeeNoticeListByTeacher.do">
    <thead>
      <tr>
  ...
  <div id=toolbar>
    ...
    <div id="dlg_outDorm" class="easyui-dialog" closed="true" style="width:430px;height:390px;padding:5px 20px"  buttons="#dlg-buttons_outDorm">
        <form id="fm_outDorm" method="post">
			<input type="hidden" name="guidval" id="notice_guid" />
