...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/themes/default/easyui.css">
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/themes/icon.css">
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/demo/demo.css">
<!-- <link rel="stylesheet" type="text/css" href="js/jquery-easyui-1.3.3/themes/bootstrap/easyui.css"> -->
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/jquery-1.8.0.min.js"
	charset="utf-8"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/jquery.easyui.min.js"
	charset="utf-8"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/jquery-easyui-1.3.2/locale/easyui-lang-zh_CN.js"
	charset="utf-8"></script>
...
<script type="text/javascript">
	...
	function deleteUser() {
		...
		if (ids.length == 1 || ids.length == 2) {
			if (selectedRows[0].student_building != null
					&& selectedRows[0].student_dorm != null
					&& selectedRows[0].student_dorm != ""
					&& selectedRows[0].student_building != "") {
				$.messager.alert("系统提示", "该学生已入住寝室，禁止删除！", "warning");
				return;
			}
		} else {
			for (var i = 0; i < selectedRows.length; i++) {
				if (selectedRows[i].student_building != null
						&& selectedRows[i].student_building != ""
						&& selectedRows[i].student_dorm != null
						&& selectedRows[i].student_dorm != "") {
					$.messager.alert("系统提示", "选中记录存在已入住学生，禁止删除！", "warning");
					return;
				}
			}
		}
		$.messager.confirm("系统提示", "您确定要删掉这<font color='red'>"
				+ selectedRows.length + "</font>条数据么?", function(r) {
			if (r) {
	...
	function newUser() {
		...
		url = "${pageContext.request.contextPath}/student/addStudent.do";
	}
	...
	function editUser() {
		...
		if (row.student_sex == "男") {
			$('input:radio[name="vo.student_sex"][value="男"]').prop('checked',
					true);
		} else if (row.student_sex == "女") {
			$('input:radio[name="vo.student_sex"][value="女"]').prop('checked',
					true);
		}
		//赋值给界面的checkBox
		...
		url = "${pageContext.request.contextPath}/student/editStudent.do?editId="
				+ row.student_id;
		;
	...
	function lackUser() {
		...
		url = "${pageContext.request.contextPath}/saveLack.do?id="
				+ row.student_id;
	}
...
<body style="margin: 5px">
	<input type="hidden" name="loginFlag" id="loginFlag"
		value="${sessionScope.loginFlag }" />
	<table id="dg" title="学生基本信息" class="easyui-datagrid"
		style="width: 900px;"
	...
	<div id="toolbar">
		<div id="systemShow" style="float: left">
			<a href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-add" plain="true" onclick="newUser()">添加学生</a> <a
				href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-edit" plain="true" onclick="editUser()">编辑学生</a> <a
				href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-remove" plain="true" onclick="deleteUser()">删除学生</a> <a
				href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-export" plain="true" onclick="exportUser()">导出学生</a>
		</div>
		<div id="teacherShow" style="float: left">
			<a href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-inDorm" plain="true" onclick="studentIndorm()">入住登记</a>
			<a href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-fenseRegister" plain="true" onclick="lackUser()">缺寝登记</a>
		...
		<div>
			<form id="condition" name="condition"
				action="${pageContext.request.contextPath}/student/exportStudent.do"
				method="post">
				&nbsp;&nbsp; 查询： <select id="s_state" class="easyui-combobox"
					name="s_state" style="width: 100px;">
					<option value="">==全部状态==</option>
					<option value="入住">入住</option>
					<option value="未入住">未入住</option>
					<option value="迁出">迁出</option>
				</select> <select class="easyui-combobox" style="width: 130px;"
					id="s_institution" name="s_institution">
				...
				</select> <input class="easyui-validatebox" style="width: 40px; height: 22;"
					id="s_class" name="s_class">班 <input
					class="easyui-validatebox" style="width: 100px; height: 22;"
					id="s_name" name="s_name" placeholder="请输入姓名"> <a
					href="javascript:void(0)" class="easyui-linkbutton"
					iconCls="icon-search" id="btn" plain="true" onclick="searchUser()">搜索</a>
			</form>
		</div>
	</div>
	...
	<div id="dlg" class="easyui-dialog"
		style="width: 420px; height: 425px; padding: 10px 20px" closed="true"
		buttons="#dlg-buttons">
		<form id="fm" action="#" method="post">
			<table cellspacing="10px;">
				<tr>
					<td>学号：</td>
					<td><input name="vo.student_userName" id="student_userName"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>姓名：</td>
					<td><input name="vo.student_name" id="student_name"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>性别：</td>
					<td>男：<input name="vo.student_sex" id="student_sex"
						type="radio" class="easyui-validatebox" checked="checked"
						required="true" value="男"> 女：<input name="vo.student_sex"
						id="student_sex" type="radio" class="easyui-validatebox"
						required="true" value="女">
					</td>
				</tr>
				<tr>
					<td>学院：</td>
					<td><input name="vo.student_institution"
						id="student_institution" class="easyui-validatebox"
						required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>专业：</td>
					<td><input name="vo.student_major" id="student_major"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>班级：</td>
					<td><input name="vo.student_class" id="student_class"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>联系电话：</td>
					<td><input name="vo.student_phone" id="student_phone"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>备注：</td>
					<td><input name="vo.student_remark" id="student_remark"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td colspan="2"><font color="red">是否为班级负责人： <input
							type="checkbox" name="vo.student_headFlag" id="student_headFlag"
							value="是"> (选中表示是)
					</font></td>
				</tr>
			</table>
	...
	<script type="text/javascript">
		...
		function studentIndorm() {
			...
			// 查询出全部的building列表
			$.post(
					"${pageContext.request.contextPath}/queryAllBuilding.do?index="
							+ Math.random(), null, callbackByBuilding);
			function callbackByBuilding(data) {
				$("#buildingData").html(data);
			}
			$("#dormData").html("<select id='dorm_id' name='dorm_id' style='width: 170'> <option value='''>==全部==</option> </select>");
		...
		function changeBuilding(id) {
			...
			if (building_id != "") {
				$.post("${pageContext.request.contextPath}/queryDormByBuilding.do?building_id=" + building_id + "&index=" + Math.random(), callbackByDorm);
			}
		}
		...
		function inDorm() {
			...
			$("#form1")
					.form(
							"submit",
							{
								url : "${pageContext.request.contextPath}/student/inDorm.do?dorm_id="
										+ dorm,
								onSubmit : function() {
									return $(this).form('validate');
								},
								success : function(result) {
									...
									if (result.errorMsg) {
										$.messager.alert("系统提示",
												result.errorMsg, "warning");
										return;
									} else {
										$.messager.alert("系统提示", "入住操作成功！",
												"info");
	...
	<!-- 入住登记 -->
	<div id="dlginDrom" class="easyui-dialog"
		style="width: 380px; height: 295px; padding: 15px 29px" closed="true"
		buttons="#dlg-buttonsinDrom">
		<form id="form1" method="post">
			<table class="table-edit" width="100%" cellspacing="10px;">
				<tr>
					<td><b>楼&nbsp;&nbsp;宇：</b> <span id="buildingData"> </span></td>
				</tr>
				<tr>
					<td><b>宿&nbsp;&nbsp;舍：</b> <span id="dormData"> <select
							id="dorm_id" name="dorm_id" style="width: 170">
								<option value="">==全部==</option>
						</select>
					</span></td>
				</tr>
				<tr>
					<td><b>学生学号：</b> <input type="text"
						id="student_userName_indorm" name="student_userName1"
						class="easyui-validatebox" disabled="disabled" /> <input
						type="hidden" id="student_userName_indorm1"
						name="student_userName" /></td>
				</tr>
				<tr>
					<td><b>学生姓名：</b> <input type="text" id="student_name_indorm"
						name="student_name" class="easyui-validatebox" disabled="disabled" />
					</td>
				</tr>
				<tr>
					<td colspan="2" align="left"><font color="red"
						style="font-size: 12">注：请选择楼宇和宿舍后确定入住！</font></td>
				</tr>
			</table>
		</form>
	</div>
	<div id="dlg-buttonsinDrom">
		<a href="javascript:void(0)" onclick="inDorm()"
			class="easyui-linkbutton" plain="true" icon="icon-search">确定入住</a>
	</div>


	<!-- 缺寝登记 -->
	<div id="dlgLack" class="easyui-dialog"
		style="width: 580px; height: 360px; padding: 20px 20px" closed="true"
		buttons="#dlg-buttonsLack">
		<form id="fmLack" method="post">
			<table cellspacing="3px;" border=1>
				<tr>
					<td style="width: 70px;">姓名：<input type="hidden"
						name="vo.student_id" id="student_id">
					</td>
					<td><input name="vo.student_name" id="student_nameLack"
						readonly="readonly" class="easyui-validatebox"
						style="width: 180px;"></td>

					<td style="width: 70px;">性别：</td>
					<td>男：<input name="vo.student_sex" id="student_sex"
						readonly="readonly" type="radio" class="easyui-validatebox"
						value="男"> 女：<input name="vo.student_sex" id="student_sex"
						readonly="readonly" type="radio" class="easyui-validatebox"
						value="女">
					</td>
				</tr>
				<tr>
					<td>学院：</td>
					<td><input name="vo.student_institution"
						id="student_institutionLack" readonly="readonly"
						class="easyui-validatebox" style="width: 180px;"></td>

					<td>专业：</td>
					<td><input name="vo.student_major" id="student_majorLack"
						readonly="readonly" class="easyui-validatebox"
						style="width: 180px;"></td>
				</tr>
				<tr>
					<td>班级：</td>
					<td><input name="vo.student_class" id="student_classLack"
						readonly="readonly" class="easyui-validatebox"
						style="width: 180px;"></td>

					<td>楼宇：</td>
					<td><input name="vo.student_building"
						id="student_buildingLack" readonly="readonly"
						class="easyui-validatebox" style="width: 180px;"></td>
				</tr>
				<tr>
					<td>宿舍：</td>
					<td><input name="vo.student_dorm" id="student_dormLack"
						readonly="readonly" class="easyui-validatebox"
						style="width: 180px;"></td>
				</tr>
				<tr>
					<td>时间段：</td>
					<td colspan=3><input id="txtBeginTime"
						name="vo.student_lackBeginTime" class="easyui-datetimebox"
						required="true"
						data-options="formatter:myformatterStart,parser:myparser"></input>
						&nbsp;至 &nbsp; <input id="txtEndTime"
						name="vo.student_lackEndTime" class="easyui-datetimebox"
						required="true"
						data-options="formatter:myformatterEnd,parser:myparser"></input></td>
				</tr>
				<tr>
					<td>原因：</td>
					<td><input name="vo.student_remark" id="student_remarkLack"
						class="easyui-validatebox" required="true" style="width: 200px;">
					</td>
				</tr>
			</table>
			<br> <font color="red">注：请填写学生缺寝时间段和原因后保存！</font>
		</form>
	</div>
	<div id="dlg-buttonsLack">
		<a href="javascript:void(0)" class="easyui-linkbutton"
			iconCls="icon-ok" onclick="saveUserLack()">保存</a> <a
			href="javascript:void(0)" class="easyui-linkbutton"
			iconCls="icon-cancel"
			onclick="javascript:$('#dlgLack').dialog('close')">关闭</a>
	</div>
	...
	<script type="text/javascript">
		...
		function studentOutDorm() {
			...
			var curr_time = new Date();
			$("#out_date_outDorm").datebox("setValue",
					myformatterOut(curr_time));
	...
	<!-- 迁出登记 -->
	<div id="dlg_outDorm" class="easyui-dialog"
		style="width: 420px; height: 360px; padding: 8px 35px" closed="true"
		buttons="#dlg-buttons_outDorm">
		<form id="fm_outDorm" method="post">
			<input type="hidden" name="vo.student_id" id="student_id_outDorm" />
			<table cellspacing="7px;">
				<tr>
					<td>学生学号：</td>
					<td><input disabled="disabled" id="student_userName_outDorm"
						class="easyui-validatebox" required="true" style="width: 200px;">
						<input type="hidden" id="student_userName_outDorm1"
						name="vo.student_userName"></td>
				</tr>
				<tr>
					<td>学生姓名：</td>
					<td><input disabled="disabled" id="student_name_outDorm"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>学生性别：</td>
					<td><input disabled="disabled" id="student_sex_outDorm"
						class="easyui-validatebox" required="true" style="width: 200px;"></td>
				</tr>
				<tr>
					<td>目前楼宇：</td>
					<td><input disabled="disabled" id="building_name_outDorm"
						class="easyui-validatebox" required="true" style="width: 200px;">
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<input disabled="disabled" id="dorm_name_outDorm"
						class="easyui-validatebox" required="true" style="width: 200px;">
					</td>
				</tr>
				<tr>
					<td>迁出日期：</td>
					<td><input type="text" id="out_date_outDorm"
						name="vo.out_date"
						data-options="formatter:myformatterOut,parser:myparserOut"
				...
				<tr>
					<td colspan="2"><font color="red" style="font-size: 13px">注：此功能只适用于毕业生,迁出后将不可恢复！</font>
					</td>
				</tr>
			</table>
		</form>
	...
	<div id="dlg-buttons_outDorm">
		<a id="ajax" href="javascript:void(0)" onclick="saveOut()"
			class="easyui-linkbutton" plain="true" icon="icon-search">确定迁出</a>
	</div>
</body>
</html>
